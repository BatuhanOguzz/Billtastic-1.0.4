        ...
        try (FileInputStream fileInputStream = new FileInputStream(excelFile);
            ...
            for (Map.Entry<String, List<String[]>> entry : groupedData.entrySet()) {
                ...
                for (String[] invoice : invoices) {
                    String dolarTutarStr = invoice[2].replaceAll("[^\\d.,]", "").replace(",", ".");
                    String tlTutarStr = invoice[4].replaceAll("[^\\d.,]", "").replace(",", ".");

                    double dolarTutar = Double.parseDouble(dolarTutarStr);
                    double tlTutar = Double.parseDouble(tlTutarStr);

                    emailBody.append("<tr>")
                            .append("<td>").append(invoice[0]).append("</td>")
                            .append("<td>").append(invoice[1]).append("</td>")
                            .append("<td>").append(dolarTutarStr).append(" $</td>")
                            .append("<td>").append(tlTutarStr).append(" ₺</td>")
                            .append("</tr>");

                    totalDolarTutar += dolarTutar;
                    totalTlTutar += tlTutar;
                }
                ...
                try {
                    ...
                    message.setFrom(new InternetAddress(username));            //ery ile değişicek
